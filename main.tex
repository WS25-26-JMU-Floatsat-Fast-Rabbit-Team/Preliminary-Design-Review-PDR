...
% --- Document Metadata ---
\title{Attitude Determination and Control System Design of Floatsat}
\subtitle{Floatsat Design Laboratory 2025/2026}
...
\begin{abstract}
This report presents the Preliminary Design Review (PDR) for the FloatSat 2025/2026 project, which aims to develop a functional floatsat prototype capable of three-degree-of-freedom attitude control on a Spherical Air Bearing Unit. The project integrates mechanical, electrical, software and control subsystems to simulate the torque-free dynamics of an orbital environment. The mechanical design features a modular, 3D-printable structure with a tilted orthogonal reaction wheel configuration, specifically optimized to align the center of gravity with the center of rotation and minimize parasitic torques. The electrical architecture utilizes a dual-microcontroller approach, employing an STM32 for control loops and sensor interfacing, alongside an ESP32 for telemetry and ground station communication. Control system analysis confirms that the selected wheel geometry provides sufficient torque authority across all principal axes, utilizing a PID strategy for initial stabilization. This document defines the system requirements and technical specifications, establishing a foundation for the subsequent design iterations, manufacturing and integration phases of the mission.
\end{abstract}
\end{@twocolumnfalse}
]
...
\begin{itemize}
    ...
    \item The Power Distribution Board shall power the Main Control Unit ($5V$).
    \item The Power Distribution Board shall power the IMU ($3.3V$).
    \item The Power Distribution Board shall power the Communication Controller ($5V$).
    \item The Power Distribution Board shall power the Motor Driver Board (Battery Voltage).
...
\begin{itemize}
	...
	\item The Main Control Unit shall be able to provide 3 different PWM signals to the drivers.
...
%-----------------------------------------
The electrical system consists of two microcontrollers, a power distribution board, an inertial measurement unit (IMU) and three motors with their drivers.
The system will be powered by an lipo battery.
The battery voltage can be monitored with an external Lipo checker.
The power distribution board will provide connections to the battery voltage as well as regulated $5V$ and $3.3V$.
The STM32F401, one of the microcontrollers, will be the main control unit.
It will read the sensor values, calculate the control loop output and control the Motor phases accordingly.
The ESP32-C6 microcontroller will be used to establish communication between the groundstation and the FloatSat.
It will provide a wireless WiFi-Link and communicate with the main control unit over SPI.
The motor drivers basically provide a half bridge for each BLDC motor phase.
The half bridge allows to drive the motor phase HIGH or LOW, while providing sufficient current flow.
Additionally each driver allows to put all three phases in a floating state.
It is not possible to put the individual phases in floating state.
This limits the driving methods for the motors.
The IMU will be connected via the I2C interface and get regulated $3.3V$ supply voltage.
Both microcontrollers will be powered over the regulated $5V$ from the power distribution board.
The drivers have their own build in LDO regulators and will be supplied the battery voltage.
Figure~\ref{fig:FloatSatSchematic} shows the detailed electrical Schematic.
...
%-----------------------------------------
The software system for the FloatSat is designed as a modular framework that enables attitude determination, reaction-wheel attitude control, and telemetry exchange while floating on a spherical air-bearing unit (SABU). Operating on the SABU requires compensating for near-frictionless motion, which demands sensor filtering, precise motor coordination, and predictable timing of the control loop on the STM32. A fast 1–10 ms loop runs on the microcontroller, using interrupts for non-blocking SPI communication and avoiding delays. The STM32F401 serves as the main control computer, handling IMU data, state estimation, and closed-loop wheel control. It is supported by an ESP32-C6, which provides a Wi-Fi link to the ground station and relays telemetry and commands over SPI.

At the lowest layer, the software provides drivers for the IMU, PWM generation, motor commutation, SPI communication, and voltage monitoring. These support mid-level services such as complementary-filter sensor fusion and reaction-wheel RPM control. The PID-based attitude controller generates torque commands, which are mapped to the tilted wheel geometry through a calibrated actuator-mapping matrix.

Higher layers include fault-detection routines to prevent motor damage or controller saturation and to recover safely from IMU or communication errors. A supervisory state controller manages modes such as startup, idle, calibration, active control, and emergency stop. The top layer handles ground-station communication and telemetry, and future iterations will try to further refine motor-control performance by measuring back-emf.
...
\section{Conclusion}
%------------------------------------------------------------------------------
The Preliminary Design Review of the FloatSat 2025/2026 project has provided a comprehensive evaluation of the system architecture and its capability to achieve three-degree-of-freedom attitude control on a Spherical Air Bearing Unit. The proposed design, characterized by a strategically tilted reaction-wheel configuration and a dual-microcontroller electronics framework, exhibits substantial potential to satisfy the project’s functional and performance requirements. The analyses conducted at this stage have elucidated the system’s strengths while also identifying areas necessitating further refinement, particularly with respect to dynamic parameter characterization, control robustness, and subsystem integration. Informed by the insights and recommendations arising from this review, the project team will undertake iterative enhancements of the mechanical, electrical, software and control subsystems to ensure the realization of a fully optimized and validated architecture in preparation for the Final Design Review.
